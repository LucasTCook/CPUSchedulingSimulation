...
static void schedule(unsigned int cpu_id)
{
    ...
    {
        context_switch(cpu_id, NULL, run);  
    }
    else  
    {
        ...
        context_switch(cpu_id, temp, run);
    }
}
...
{
    pthread_mutex_lock(&current_mutex);
    while(pthread_cond_wait(&idle_cond, &current_mutex));  
    pthread_mutex_unlock(&current_mutex);
    schedule(cpu_id);
...
{
    pthread_mutex_lock(&current_mutex);  //create mutex lock for array of processes
    current[cpu_id]->state = PROCESS_WAITING;  //set process at index cpu_id to waiting
    pthread_mutex_unlock(&current_mutex);  //end lock
    schedule(cpu_id);  //call schedule with cpu_id
}
...
int main(int argc, char *argv[])
{
    ...
    if (!(argc % 2 == 0 && argc < 5))  //check if the amount of arguments is within the range
    {
